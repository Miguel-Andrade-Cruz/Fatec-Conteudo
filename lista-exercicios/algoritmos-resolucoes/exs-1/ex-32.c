...
int main() {
    ...
    Inputer data[] = {
        {.msg = "Qual é o seu peso? \n", .type = T_FLOAT},
        {.msg = "Qual é a sua altura? \n", .type = T_FLOAT}
    };
    InputerSet *user_data = group_inputs(data, 2);
    
    Validator *constraints[] = {
        greater_than(0)
    };
    Needs *constraints_group = group_needs(constraints, 1);
    
    pull(user_data, constraints_group);
    
    
    float calculated_imc = takefloat(user_data->arr[0]) / ( takefloat(user_data->arr[1]) * takefloat(user_data->arr[1]) );
    ...
    clean_all(user_data, constraints_group);
    return 0;
}